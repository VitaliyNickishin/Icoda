...
if( $_POST['lang-source'] == 'es' || $_POST['lang-source'] == 'zh-hans' ) {
    if( $_POST['lang-source'] == 'es' ) {
        $responsible_es_users = array('6115717', '8007289');
        $last_responsible_user_id = get_option( 'last_responsible_es_user_id' );
        ...
        update_option( 'last_responsible_es_user_id', $last_responsible_user_id );

    }
    if( $_POST['lang-source'] == 'de' ) {
        $last_responsible_user_id = '6357817';
    }
    ...
    $last_responsible_user_id = get_option( 'last_responsible_user_id' );
    if( empty( $last_responsible_user_id ) ) {
    ...
    update_option( 'last_responsible_user_id', $last_responsible_user_id );
}

file_put_contents( __DIR__ . '/process_log.txt', print_r("START  " . date('Y-m-d H:i:s') . "\n", true) );
define('TOKEN_FILE', __DIR__ . DIRECTORY_SEPARATOR . 'token_info.json');

use AmoCRM\OAuth2\Client\Provider\AmoCRM;
file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 1" . "\n", true), FILE_APPEND );
include_once __DIR__ . '/vendor/autoload.php';
include_once __DIR__ . '/vendor/amocrm/oauth2-amocrm/src/AmoCRM.php';
file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 2" . "\n", true), FILE_APPEND );
session_start();
...
file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 3" . "\n", true), FILE_APPEND );
$accessToken = getToken();

$provider->setBaseDomain($accessToken->getValues()['baseDomain']);
file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 4" . "\n", true), FILE_APPEND );

/**
 * Проверяем активен ли токен и делаем запрос или обновляем токен
 */
if ($accessToken->hasExpired()) {
    file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 5" . "\n", true), FILE_APPEND );

    /**
     * Получаем токен по рефрешу
     */
    try {
        file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 6" . "\n", true), FILE_APPEND );

        file_put_contents( __DIR__ . '/process_log.txt', print_r("getRefreshToken  =  " . $accessToken->getRefreshToken() . "\n", true), FILE_APPEND );

        file_put_contents( __DIR__ . '/process_log.txt', print_r("accessToken  =  " . $accessToken->getToken() . "\n", true), FILE_APPEND );
        file_put_contents( __DIR__ . '/process_log.txt', print_r("expires  =  " . $accessToken->getExpires() . "\n", true), FILE_APPEND );
        file_put_contents( __DIR__ . '/process_log.txt', print_r("baseDomain  =  " . $provider->getBaseDomain() . "\n", true), FILE_APPEND );

        $accessToken = $provider->getAccessToken(new League\OAuth2\Client\Grant\RefreshToken(), [
            'refresh_token' => $accessToken->getRefreshToken(),
        ]);

        file_put_contents( __DIR__ . '/process_log.txt', print_r("access token = " . $accessToken . "\n", true), FILE_APPEND );
...
try {
    file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 8" . "\n", true), FILE_APPEND );
    ...
    $utm_fields = array( 'utm_source' => '589987', 'utm_medium' => '589989', 'utm_campaign' => '589991', 'utm_content' => '589993', 'utm_term' => '589995' );
    ...
    $data = $provider->getHttpClient()
        ->request('POST', $provider->urlAccount() . 'api/v2/leads', [
            ...
            'body' => json_encode( array(
                "add" => array(
                    array(
                        "name" => 'Заявка с сайта ' . $moscowDate,
    ...
    file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 9" . "\n", true), FILE_APPEND );

} catch (GuzzleHttp\Exception\GuzzleException $e) {
    file_put_contents( __DIR__ . '/process_log.txt', print_r("Step 10" . "\n", true), FILE_APPEND );
    return;
}
file_put_contents( __DIR__ . '/process_log.txt', print_r("END" . "\n", true), FILE_APPEND );
