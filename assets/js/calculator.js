jQuery(document).ready(function ($) {
    ...
    $('body').on('click', '.btn-apply', function () {
        ...
        if($('input[name="project-name"]').val().length < 1
        ...
        $('.btn-prew, .btn-next, .btn-apply').addClass('d-none')
        currentStep = 'finish'
        updateProgressBar()
    });
    ...
    function checkCheckbox() {
        ...
        check.change(function() {
            ...
            boxWrapper.find('.checkbox').not(box).not('.multi-choise').prop({checked: false});

            const checkParent = boxWrapper.find('.checkbox').not('.multi-choise').parent();
